...
const MobileNav = ({ isOpen, toggleDrawer }: MobileNavProps) => {
  const pathname = usePathname();
  const [active, setActive] = useState(1);

  return (
    <Drawer
      customIdSuffix="mobile-nav"
      open={isOpen}
      onClose={toggleDrawer}
      direction="right"
      overlayOpacity={0.6}
    >
      <div className="mobile-menu fixed top-0 w-full bg-flashWhite dark:bg-nightBlack z-999 h-full xl:hidden transition-all duration-300 [&.is-menu-open]:right-0 py-8 px-6 overflow-y-auto">
        <button
