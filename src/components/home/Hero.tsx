      ...
      <div className="px-5 py-8 bg-white dark:bg-nightBlack rounded-2xl hero-section md:p-8 lg:p-10 2xl:p-13">
        <div className="inline-flex items-center gap-2 px-4 py-2 text-xs tracking-wide text-black dark:text-white border lg:px-5 section-name border-platinum dark:border-greyBlack200 rounded-4xl">
          {introduce?.iconBox?.Icon && (
            typeof introduce.iconBox.Icon === 'function' 
              ? React.createElement(introduce.iconBox.Icon)
              : introduce.iconBox.Icon
          )}
          {introduce?.iconBox?.title}
        </div>
        <div className="items-center gap-6 hero-content md:flex xl:gap-10">
          <div className="hero-image flex-[0_0_20.3rem] md:hidden  flex justify-center items-center">
            <OptimizedImage
              src={heroImg}
              width={350}
              height={200}
          ...
          <div className="text-content pt-7 lg:pt-8 max-lg:max-w-[30rem]">
            <h1 className="text-[30px] lg:text-3xl xl:text-3xl 2xl:text-5xl font-semibold text-black dark:text-white leading-1.27 lg:leading-1.27 xl:leading-1.27 2xl:leading-1.27 mb-4 lg:mb-5">
